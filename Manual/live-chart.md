...
- 기본값: 최근 3개월
- `From` 변경 시 차트 timeScale이 자동으로 새 윈도우에 맞춰 fit됨. DB에 더 과거 봉이 있다면 그만큼 끌어와 표시함 (store-side 90일 cap 제거됨, 2026-05-17)
- 라이브 tick으로 마지막 봉만 갱신될 때는 fit이 일어나지 않아 사용자가 잡은 줌/팬이 보존됨
